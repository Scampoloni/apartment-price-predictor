...
Never retrains; only calls price_estimator.src.predict.predict_price().
...
from price_estimator.src.predict import predict_price, _load_metadata

# Offer only municipalities observed during training. Custom values remain
# possible, but they receive an explicit unseen-location warning.
ZH_MUNICIPALITIES = [""] + _load_metadata().get("known_municipalities", [])
...
def predict_fn(
    ...
    try:
        ...
        if result.get("municipality_warning"):
            note += f"\n\n⚠️ {result['municipality_warning']}"
        ...
        error_md = (
            ...
            "<code>python -m price_estimator.src.train --iteration 2</code>"
            "</div>"
        )
        return error_md, ""
...
def _model_info_md() -> str:
    ...
    return (
        ...
        f"MAE CHF {meta.get('holdout_mae', '–'):,}, "
        f"R² {meta.get('holdout_r2', '–')}  \n"
        f"**Municipality GroupKFold:** RMSE CHF {geographic.get('rmse', '–'):,}, "
        f"MAE CHF {geographic.get('mae', '–'):,}, "
        f"R² {geographic.get('r2', '–')}  \n"
        f"**Dataset:** {dataset.get('usable_rows', '–')} usable listings, "
        f"{dataset.get('municipalities', '–')} municipalities  \n"
...
with gr.Blocks(title="Zurich Apartment AI Suite — Price Estimator") as demo:

    gr.Markdown("""
    # Zurich Apartment AI Suite — Price Estimator
    ...
    > Research prototype trained on a small canton-only listing sample.
    > Predictions are uncertain statistical estimates, not valuations or offers.
    """)
